...
def _parse_cad_pdf(pdf_bytes: bytes) -> dict:
    ...
    if len(drawings) >= 10:
        ...
        if ret:
            ...
            if text_scale:
                result["scale_mm_per_px"] = text_scale
                result["scale_confirmed"] = True
                result["detected_width_mm"] = text_scale * TARGET_W
                result["detected_height_mm"] = text_scale * TARGET_H
                logger.info(f"[parser] 스케일(치수 텍스트): {text_scale:.2f} mm/px, "
                            f"실제 크기={text_scale * TARGET_W:.0f}x{text_scale * TARGET_H:.0f}mm")
            else:
            ...
            scale = _extract_scale_from_text(page)
            if scale:
                result["scale_mm_per_px"] = scale
                result["scale_confirmed"] = True
    else:
...
def _extract_scale_from_text(page) -> Optional[float]:
    """PDF 텍스트 치수선 → scale_mm_per_px.

    전략:
    1. 3~6자리 숫자 추출 (mm 치수)
    2. '1:숫자' 축척 표기 탐지
    3. 'mm', 'cm', 'm' 단위 동반 숫자 탐지
    """
    text = page.get_text()

    # 전략 1: 단위 동반 숫자 (12000mm, 12m, 1200cm)
    ...
    logger.info(f"[parser] scale: {max_num}mm / {TARGET_W}px = {scale:.2f} mm/px")
    return scale
