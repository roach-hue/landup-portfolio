...
# dead_zone 통합 (타입 구분 없이 배치 불가 영역)
...
# dead_zone 세분류: core(화장실/계단 — 진입로 확보 필요) vs pillar(기둥)
...
def run(state: SmallState) -> SmallState:
    """DXF 파일 파싱 → floor_polygon_px + 설비 + 입구.

    레이어명 규칙(usable_poly, entrance_zone, dead_zone_core, dead_zone_pillar,
    mep_sprinkler, mep_power)이 있으면 레이어 기반 직접 추출.
    ...
    # ── 레이어명 확인: 우리 규칙대로 그려진 도면인지 판별 (별칭 포함) ──
    ...
    inner_walls   = [
        {"start_px": (w["start_px"][0], _fy(w["start_px"][1])),
         "end_px":   (w["end_px"][0],   _fy(w["end_px"][1]))}
...
LAYER_ALIASES = {
    ...
    # 코어 세분화: 계단/화장실 분리 (레이어별 다른 물리 규칙 적용)
    ...
    # 하위 호환: dead_zone_core → core_toilet로 취급 (기존 도면)
...
def _standardize_layer(raw_name: str) -> str | None:
...
def _parse_by_layer(msp) -> dict:
    ...
    레이어 규칙 (별칭 포함):
      ...
      entrance_zone     → 입구 영역 (LINE)
      dead_zone_core    → 화장실/계단 (LWPOLYLINE)
    ...
    logger.info(f"[parser_dxf:layer] polygon coords: {floor_polygon}")
    logger.info(f"[parser_dxf:layer] X range: {min(xs):.0f}~{max(xs):.0f}, Y range: {min(ys):.0f}~{max(ys):.0f}")
...
def _extract_inaccessible(msp, ox, oy, segments):
    ...
    for entity in msp.query("TEXT MTEXT"):
        ...
        if enclosing:
            results.append({"polygon_px": enclosing, "confidence": "high"})
        else:
