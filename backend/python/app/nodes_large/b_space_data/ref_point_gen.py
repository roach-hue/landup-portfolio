...
# 대표점 생성 기준: 세그먼트 길이가 이 이상이면 삼등분점도 추가
THIRDS_THRESHOLD = 2000
...
# 2026-05-04: 1200 → 2200 으로 확대.
# 이유: interior grid 의 min_wall_dist (= step * 0.X, 동적) 가 큰 도면에서 step=2000 → min_wall_dist=1600 까지 박혀서
# 기존 CENTER_MIN_WALL_DIST=1200 보다 커지는 모순 발생 (interior 영역 = 빈 집합).
# step=2000 도면에서도 interior 영역 (min_wall_dist ~ CENTER_MIN_WALL_DIST 사이) 확보되도록 확대.
CENTER_MIN_WALL_DIST = 2200
...
def _generate_interior_ref_points(
    ...
    # 2026-05-04: step * 0.8 → step * 0.4 로 축소.
    # 이유: step=2000 (큰 도면) 에서 min_wall_dist=1600 이 CENTER_MIN_WALL_DIST=1200 (구) 보다 커서 interior 영역 = 빈 집합.
    # 0.4 로 축소하면 step=2000 → min_wall_dist=800. CENTER_MIN_WALL_DIST=2200 (신) 와 합쳐 interior 영역 = 800~2199.
    min_wall_dist = step * 0.4
    ...
    for gx in frange(minx + step, maxx - step, step):
        for gy in frange(miny + step, maxy - step, step):
            ...
            if point_near_any(pt, entrance_points_shapely, DECOMPRESSION_RADIUS_MM):
                continue
