"""
intent_parser 노드 — 대형·야외 파이프라인 (Shin).
...
def run(state: LargeState) -> LargeState:
    ...
    if not user_requirements.strip():
        logger.info("[intent_parser:large] user_requirements 없음 — 건너뜀")
    ...
    try:
        ...
        logger.error(f"[intent_parser:large] LLM 호출 실패 — 기존 배치 유지: {e}")
        return {"resolved_intents": [], "intent_parse_error": str(e)}

    serialized = [dataclasses.asdict(i) for i in intents]
    logger.info(f"[intent_parser:large] {len(serialized)}개 인텐트 → state['resolved_intents']")
